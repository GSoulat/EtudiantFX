...
public class McListe implements Initializable {
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		FilteredList<Etudiant> filteredData = new FilteredList<>(list, b -> true);
		filterField.textProperty().addListener((observable, oldValue, newValue) -> {
			filteredData.setPredicate(etudiant -> {
				...
				if (etudiant.getPrenom().toLowerCase().indexOf(lowerCaseFilter) != -1) {
					return true;
				} else if (etudiant.getNom().toLowerCase().indexOf(lowerCaseFilter) != -1) {
					return true;
				} else if (String.valueOf(etudiant.getIdEtudiant()).indexOf(lowerCaseFilter) != -1) {
					return true;
				} else if (String.valueOf(etudiant.getDateNaissance()).indexOf(lowerCaseFilter) != -1) {
					return true;
				} else
					return false;
			});
	...
	public void choice1(ActionEvent event) {
		...
		paneListe.getChildren().setAll(root);
	};
				...
				final TableCell<Etudiant, Void> cell = new TableCell<Etudiant, Void>() {
...

					{
						btn.setOnAction((ActionEvent event) -> {
							...
							AnchorPane root = null;
							try {
								root = FXMLLoader.load(getClass().getResource("Modifie.fxml"));
							} catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
